# coding=utf-8
"""
This module bundles all of RaionPi's supported plugin implementation types as well as their common parent
...
class OctoPrintPlugin(Plugin):
	"""
	The parent class of all RaionPi plugin mixins.
...
class StartupPlugin(OctoPrintPlugin):
	"""
	The ``StartupPlugin`` allows hooking into the startup of RaionPi. It can be used to start up additional services
...
class ShutdownPlugin(OctoPrintPlugin):
	"""
	The ``ShutdownPlugin`` allows hooking into the shutdown of RaionPi. It's usually used in conjunction with the
	...
	def on_shutdown(self):
		"""
		Called upon the imminent shutdown of RaionPi.
		"""
		pass
...
class AssetPlugin(OctoPrintPlugin, RestartNeedingPlugin):
	...
	def get_assets(self):
		...
		The assets will be made available by RaionPi under the URL ``/plugin/<plugin identifier>/static/<path>``, with
		``plugin identifier`` being the plugin's identifier and ``path`` being the path as defined in the asset dictionary.

		Assets of the types ``js``, ``css`` and ``less`` will be automatically bundled by RaionPi using
...
class TemplatePlugin(OctoPrintPlugin, ReloadNeedingPlugin):
	"""
	Using the ``TemplatePlugin`` mixin plugins may inject their own components into the RaionPi web interface.

	Currently RaionPi supports the following types of injections out of the box:
	...
	the "helloworld" plugin in RaionPi's :ref:`Plugin Tutorial <sec-plugins-gettingstarted>`.
...
class SimpleApiPlugin(OctoPrintPlugin):
	...
	The simple API constructed by RaionPi for you will be made available under ``/api/plugin/<plugin identifier>/``.
	RaionPi will do some preliminary request validation for your defined commands, making sure the request body is in
	...
	def on_api_command(self, command, data):
		"""
		Called by RaionPi upon a POST request to ``/api/plugin/<plugin identifier>``. ``command`` will contain one of
		...
		If your plugin returns nothing here, RaionPi will return an empty response with return code ``204 No content``
		...
		:return: ``None`` in which case RaionPi will generate a ``204 No content`` response with empty body, or optionally
	...
	def on_api_get(self, request):
		"""
		Called by RaionPi upon a GET request to ``/api/plugin/<plugin identifier>``. ``request`` will contain the
...
class BlueprintPlugin(OctoPrintPlugin, RestartNeedingPlugin):
	...
	Your blueprint will be published by RaionPi under the base URL ``/plugin/<plugin identifier>/``, so the above
...
class SettingsPlugin(OctoPrintPlugin):
	"""
	Including the ``SettingsPlugin`` mixin allows plugins to store and retrieve their own settings within RaionPi's
	configuration.
	...
	def on_settings_save(self, data):
		...
		additional settings properties that are not stored within RaionPi's configuration.
	...
	def get_settings_version(self):
		...
		Use this to have RaionPi trigger your migration function if it detects an outdated settings version in
		config.yaml.
	...
	def on_settings_migrate(self, target, current):
		"""
		Called by RaionPi if it detects that the installed version of the plugin necessitates a higher settings version
		...
		Your plugin's implementation should take care of migrating any data by utilizing self._settings. RaionPi
	...
	def on_settings_cleanup(self):
		...
		version of RaionPi that still duplicated default data).
		"""
		import octoprint.util
...
class EventHandlerPlugin(OctoPrintPlugin):
	"""
	The ``EventHandlerPlugin`` mixin allows RaionPi plugins to react to any of :ref:`RaionPi's events <sec-events>`.
	RaionPi will call the :func:`on_event` method for any event fired on its internal event bus, supplying the
	event type and the associated payload. Please note that until your plugin returns from that method, further event
	processing within RaionPi will block - the event queue itself is run asynchronously from the rest of RaionPi,
	...
	def on_event(self, event, payload):
		"""
		Called by RaionPi upon processing of a fired event on the platform.
...
class SlicerPlugin(OctoPrintPlugin):
	"""
	Via the ``SlicerPlugin`` mixin plugins can add support for slicing engines to be used by RaionPi.

	"""

	def is_slicer_configured(self):
		"""
		Unless the return value of this method is ``True``, RaionPi will not register the slicer within the slicing
	...
	def get_slicer_properties(self):
		...
		same_device
		    True if the slicer runs on the same device as RaionPi, False otherwise. Slicers running on the same
		...
		progress_report
		    ``True`` if the slicer can report back slicing progress to RaionPi ``False`` otherwise.
	...
	def do_slice(self, model_path, printer_profile, machinecode_path=None, profile_path=None, position=None, on_progress=None, on_progress_args=None, on_progress_kwargs=None):
		"""
		Called by RaionPi to slice ``model_path`` for the indicated ``printer_profile``. If the ``machinecode_path`` is ``None``,
		slicer implementations should generate it from the provided ``model_path``.

		If provided, the ``profile_path`` is guaranteed by RaionPi to be a serialized slicing profile created through the slicing
		...
		Please note that both ``on_progress_args`` and ``on_progress_kwargs`` as supplied by RaionPi might be ``None``,
...
class ProgressPlugin(OctoPrintPlugin):
	...
	def on_print_progress(self, storage, path, progress):
		"""
		Called by RaionPi on minimally 1% increments during a running print job.
	...
	def on_slicing_progress(self, slicer, source_location, source_path, destination_location, destination_path, progress):
		"""
		Called by RaionPi on minimally 1% increments during a running slicing job.
