		...
		try:
			...
			try:
				...
				if "__version" in data:
					...
					self._logger.info("Can't determine version of RaionPi version cache was created for, not using it")
					return

				from octoprint._version import get_versions
				octoprint_version = get_versions()["version"]
				if data_version != octoprint_version:
					self._logger.info("Version cache was created for another version of RaionPi, not using it")
					return
...
__plugin_description__ = "Allows receiving update notifications and performing updates of RaionPi and plugins"
