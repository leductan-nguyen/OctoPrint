...
__plugin_description__ = "Makes the RaionPi instance discoverable via Bonjour/Avahi/Zeroconf and uPnP"
...
class DiscoveryPlugin(octoprint.plugin.StartupPlugin,
	...
	##~~ StartupPlugin API -- used for registering RaionPi's Zeroconf and SSDP services upon application startup
	...
	##~~ ShutdownPlugin API -- used for unregistering RaionPi's Zeroconf and SSDP service upon application shutdown
	...
	def zeroconf_register(self, reg_type, name=None, port=None, txt_record=None):
		...
		:param name: displayable name of the service, if not given defaults to the RaionPi instance name
		:param port: port to register for the service, if not given defaults to RaionPi's (public) port
	...
	def zeroconf_unregister(self, reg_type, port=None):
		...
		:param port: the port of the service to be unregistered, defaults to RaionPi's (public) port if not given
		:return:
		"""
	...
	def _create_http_txt_record_dict(self):
		"""
		Creates a TXT record for the _http._tcp Zeroconf service supplied by this RaionPi instance.
	...
	def _create_octoprint_txt_record_dict(self):
		"""
		Creates a TXT record for the _octoprint._tcp Zeroconf service supplied by this RaionPi instance.

		The following keys are defined:

		  * `path`: path prefix to actual RaionPi instance, inherited from _http._tcp
		  ...
		  * `version`: RaionPi software version
		  * `api`: RaionPi API version
		  * `model`: Model of the device that is running RaionPi
		  * `vendor`: Vendor of the device that is running RaionPi
	...
	def _ssdp_register(self):
		"""
		Registers the RaionPi instance as basic service with a presentation URL pointing to the web interface
		"""
	...
	def _ssdp_unregister(self):
		"""
		Unregisters the RaionPi instance again
		"""
	...
	def get_instance_name(self):
		...
		if name:
			return u"RaionPi instance \"{}\"".format(name)
		else:
			import socket
			return u"RaionPi instance on {}".format(socket.gethostname())
