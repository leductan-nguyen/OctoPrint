...
def main():
	...
	parser.add_argument("-v", "--version", action="store_true", dest="version",
						help="Output RaionPi's version and exit")
	...
	parser.add_argument("-c", "--config", action="store", dest="config",
						help="Specify the config file to use. RaionPi needs to have write access for the settings dialog to work. Defaults to ~/.octoprint/config.yaml")
	parser.add_argument("-b", "--basedir", action="store", dest="basedir",
						help="Specify the basedir to use for uploads, timelapses etc. RaionPi needs to have write access. Defaults to ~/.octoprint")
	...
	parser.add_argument("--daemon", action="store", type=str, choices=["start", "stop", "restart"],
						help="Daemonize/control daemonized RaionPi instance (only supported under Linux right now)")
	...
	parser.add_argument("--iknowwhatimdoing", action="store_true", dest="allowRoot",
						help="Allow RaionPi to run as user root")

	args = parser.parse_args()

	if args.version:
		print "RaionPi version %s" % __version__
		sys.exit(0)
