...
class Server(object):
	...
	def run(self):
		...
		self._logger.info("Starting RaionPi %s" % DISPLAY_VERSION)
		...
		try:
			# this is the main loop - as long as tornado is running, RaionPi is running
			ioloop.start()
		except (KeyboardInterrupt, SystemExit):
			pass
		except:
			self._logger.fatal("Now that is embarrassing... Something really really went wrong here. Please report this including the stacktrace below in RaionPi's bugtracker. Thanks!")
	...
	def _check_for_root(self):
		if "geteuid" in dir(os) and os.geteuid() == 0:
			exit("You should not run RaionPi as root!")
