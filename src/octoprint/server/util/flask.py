...
def restricted_access(func):
	...
	done for RaionPi's Access Control plus that any conditions of the
	...
	If RaionPi's Access Control has not been setup yet (indicated by the "firstRun"
	...
	def decorated_view(*args, **kwargs):
		# if RaionPi hasn't been set up yet, abort
		if settings().getBoolean(["server", "firstRun"]) and settings().getBoolean(["accessControl", "enabled"]) and (octoprint.server.userManager is None or not octoprint.server.userManager.hasBeenCustomized()):
			return flask.make_response("RaionPi isn't setup yet", 403)
